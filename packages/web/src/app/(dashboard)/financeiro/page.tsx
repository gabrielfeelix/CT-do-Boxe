            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-5">
                <div>
                    <h2 className="text-2xl font-black text-gray-900 tracking-tight flex gap-2 items-center">
                        <PiggyBank className="w-6 h-6 text-emerald-500" /> Financeiro
                    </h2>
                    <p className="text-sm font-bold text-gray-400 uppercase tracking-widest mt-1">Resumo de {new Date().toLocaleString('pt-BR', { month: 'long' })}</p>
                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden h-fit">
                            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-50/80 bg-gray-50/50">
                                <h3 className="text-lg font-black text-gray-900 flex items-center gap-2"><CreditCard className="w-5 h-5 text-gray-400" /> Últimos pagamentos</h3>
                            </div>
                            ...
                            {pagamentos.length === 0 ? (
                                <div className="py-20 text-center flex flex-col items-center">
                                    ...
                                    <p className="text-sm font-bold text-gray-900">Nenhum pagamento registrado.</p>
                                    <p className="text-xs font-medium text-gray-500 max-w-xs mt-1">Os pagamentos do mês aparecerão aqui.</p>
                                </div>
                            ) : (
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-white border-b border-gray-100">
                                            <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest w-1/2">Aluno</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest hidden sm:table-cell">Vencimento</th>
                        ...
                        <div className="space-y-6">
                            {inadimplentes.length > 0 && (
                                <div className="bg-gradient-to-br from-red-50 to-orange-50/50 border border-red-200/50 rounded-3xl p-6 shadow-sm relative overflow-hidden">
                                    ...
                                    <div className="flex items-center justify-between mb-5 relative z-10">
                                        ...
                                        <Link href="/financeiro/inadimplencia" className="text-xs font-bold text-red-600 hover:text-red-800 uppercase tracking-wider flex items-center bg-white px-2.5 py-1 rounded-md shadow-sm border border-red-100 transition-colors">Ver todos <ChevronRight className="w-3 h-3 ml-0.5" /></Link>
                                    </div>
                                            ...
                                            return (
                                                <div key={p.id} className="flex items-center justify-between bg-white rounded-2xl px-5 py-4 shadow-sm border border-red-100 hover:border-red-300 transition-colors">
                                                    <div className="min-w-0 pr-3">
                                                        ...
                                                        <p className="text-[10px] uppercase font-bold text-red-500 mt-1">{dias} dias em atraso</p>
                                                    </div>
                            ...
                            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Resumo do mês</p>
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-600">Pagos</span>
                                        <span className="text-sm font-bold text-emerald-700">{formatCurrency(totalPago)}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-600">Pendentes</span>
                                        <span className="text-sm font-bold text-blue-700">{formatCurrency(totalPendente)}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm font-medium text-gray-600">Em atraso</span>
                                        <span className="text-sm font-bold text-red-700">{formatCurrency(totalEmAberto)}</span>
                                    </div>
                                    <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
                                        <span className="text-sm font-bold text-gray-900">Total esperado</span>
                                        <span className="text-sm font-black text-gray-900">{formatCurrency(totalPago + totalPendente)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
