...
type AlunoSelecionavel = Pick<Aluno, 'id' | 'nome' | 'email' | 'cpf'>
...
function NovoContratoForm() {
    ...
    useEffect(() => {
        ...
        async function fetchAlunoSelecionado() {
            const { data } = await supabase.from('alunos').select('id,nome,email,cpf').eq('id', alunoIdParam).single()
            if (data) setAlunoSelecionado(data as AlunoSelecionavel)
        }
        fetchAlunoSelecionado()
    }, [supabase, alunoIdParam])
    ...
    }, [supabase, alunoBusca, alunoSelecionado])
            ...
            <section className="space-y-6 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                    ...
                    <button
                        ...
                        disabled={saving || !alunoSelecionado || !planoSelecionado}
...
export default function NovoContratoPage() {
    return (
        <Suspense fallback={<div className="mx-auto max-w-3xl py-20"><LoadingSpinner label="Carregando formulário..." /></div>}>
            <NovoContratoForm />
        </Suspense>
    )
}
